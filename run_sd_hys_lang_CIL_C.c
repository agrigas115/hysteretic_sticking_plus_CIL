...
void compute_adhesion_repulsion(double coords[][2], int nonbonded_array1[], int nonbonded_array2[], int num_nonbonded, 
	...
	double rand_gauss1, rand_gauss2, theta;
	...
	for (i=0;i<num_nonbonded;i++){
		...
		if (adhesion_array[i] == 1){
			...
			ar_force[nonbonded_array1[i]][0] += delta_x_norm;
			ar_force[nonbonded_array1[i]][1] += delta_y_norm;
			ar_force[nonbonded_array2[i]][0] += -delta_x_norm;
			ar_force[nonbonded_array2[i]][1] += -delta_y_norm;

			adhesion_count_array[nonbonded_array1[i]] += 1;
			adhesion_count_array[nonbonded_array2[i]] += 1;
	...
	for (i=0;i<num_atoms;i++){
		num = adhesion_count_array[i];
		if (num > 0){
			x1 = ar_force[i][0];
			y1 = ar_force[i][1];
			distance = sqrt((x1*x1) + (y1*y1));
			ar_force[i][0] /= distance;
			ar_force[i][1] /= distance;
		}
	}
	...
	for (i=0;i<num_atoms;i++){
		...
		x1 -= Lx * nearbyint(x1 / Lx);
		y1 -= Ly * nearbyint(y1 / Ly);
...
int main(int argc, char *argv[]) {
	...
	int adhesion_count_array[num_atoms];
...
//////////
	...
	snprintf(buf, sizeof(buf), "chk/coords_%s_p%s_v0_%s_phi%s_T_%s_gamma_%s_seq_%s.chk",argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
	fp = fopen(buf, "r");
	...
	if (fp){
		...
		// Load adhesion array
		snprintf(buf, sizeof(buf), "chk/adhesion_array_%s_p%s_v0_%s_phi%s_T_%s_gamma_%s_seq_%s.chk",argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
		fp = fopen(buf, "r");
		for (i=0; i<num_nonbonded; i++){
		...
		snprintf(buf, sizeof(buf), "vstress_data/vstress_%s_p%s_v0_%s_phi%s_T_%s_gamma_%s_seq_%s.txt",argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
		remove(buf);
		snprintf(buf, sizeof(buf), "bonded_data/bonded_%s_p%s_v0_%s_phi%s_T_%s_gamma_%s_seq_%s.txt",argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
		remove(buf);
		snprintf(buf, sizeof(buf), "traj_data/traj_%s_p%s_v0_%s_phi%s_T_%s_gamma_%s_seq_%s.txt",argv[1],argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);
		remove(buf);
	...
	while (NVT_count < 1e7){
        ...
        memset(adhesion_count_array, 0, sizeof(adhesion_count_array));
		...
		avg_delta = compute_slj(coords, nonbonded_array1, nonbonded_array2, total_force, num_neighs, vlist, Lx, Ly, vstress, sigma_ij_array, num_atoms, adhesion_array, &z);
		compute_adhesion_repulsion(coords, nonbonded_array1, nonbonded_array2, num_nonbonded, total_force, adhesion_array, v0, Lx, Ly, ar_force, num_atoms, vstress_a, adhesion_count_array);
